...
int main(void) {
    ...
    FILE *Fc = fopen("920 r=0.2 delta=0.03 beta=0.32 direct_time.csv", "w");  // 打开文件以保存合作率数据
    ...
    for(int i = 0; i <= Round; i++){
	    ...
	    for (int lo =0; lo<loop; lo++) {
			...
			dp_temp = data_out[2];
			dn_temp = data_out[3];
			record_loop[lo][0] = (double)cp_temp;
			record_loop[lo][1] = (double)cn_temp;
			record_loop[lo][2] = (double)dp_temp;
			record_loop[lo][3] = (double)dn_temp;

			printf("%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",i,R,delta,beta,(double)cp_temp,(double)cn_temp,(double)dp_temp,(double)dn_temp);
	    }

	    cal_dev();
	    fprintf(Fc,"%d,%f,%f,%f,%f,%f,%f,%f\n",i,R,delta,beta,mean_temp_cp,mean_temp_cn,mean_temp_dp,mean_temp_dn);
